...
class ExamplesTest extends FunSuite {
  ...
  test("nightly backup at 2:30 AM") {
    assertEquals(Schedule.daily.at(2.h).at(30.min).toCron, "30 2 * * *")
  }
  ...
  test("stand-up reminder on Monday and Friday at noon") {
    assertEquals(
      Schedule.on(Mon, Fri).at(12.h).at(0.min).toCron,
      "0 12 * * 1,5"
    )
  }
  ...
  test("summer newsletter in June and July, daily at 9 AM") {
    assertEquals(
      Schedule.in(Month.June, Month.July).daily.at(9.h).at(0.min).toCron,
      "0 9 * 6,7 *"
    )
  }
  ...
  test("deadline reminder on June 15th at 9 AM") {
    assertEquals(
      Schedule.in(Month.June).onThe(15.th).at(9.h).at(0.min).toCron,
      "0 9 15 6 *"
    )
  }
  ...
  test("cleanup on the 1st of the month or any Friday at 4:30 AM") {
    assertEquals(
      Schedule.onThe(1.st).orOn(Fri).at(4.h).at(30.min).toCron,
      "30 4 1 * 5"
    )
    assertEquals(
      Schedule.onThe(1.st).orOn(Fri).at(4.h).at(30.min).humanReadable,
  ...
  test("compile-error snippets fail on the phase types, not lost scope") {
    assertEquals(compileErrors("Schedule.daily.at(9.h).at(0.min)"), "")
